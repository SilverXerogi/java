...
public class MenuFactory {
    ...
    private final Inventory inventory; // Добавлено
    private final Scanner scanner = new Scanner(System.in);

    public MenuFactory(BookStoreService store) {
        this.store = store;
        this.inventory = Inventory.getInstance(); // Получаем инстанс инвентаря
    }
        ...
        switch (type) {
            ...
            case REPORTS:
                return new Menu("Отчёты", Arrays.asList(
                        // --- Изменено: теперь использует метод инвентаря, который читает из конфига ---
                        new MenuItem("Залежавшиеся книги (" + inventory.getStaleMonthsThreshold() + " мес)", () ->
                                inventory.listStaleBooks().forEach(System.out::println), null), // Вызов без параметра
                        // --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
