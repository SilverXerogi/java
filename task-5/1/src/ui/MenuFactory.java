            ...
            case BOOKS:
                return new Menu("Меню книг", Arrays.asList(
                        ...
                        new MenuItem("Добавить книгу", () -> {
                            ...
                            // используем фабрику для генерации книги с авто-id
                ...
                return new Menu("Меню заказов", Arrays.asList(
                        new MenuItem("Создать заказ", () -> {
                            ...
                            while (true) {
                                ...
                                System.out.println("\nДоступные книги:");
                                books.forEach(b -> System.out.println(b.getId() + " — " + b.getTitle()
                                        + " (цена: " + b.getPrice() + " руб, статус: " + b.getStatus() + ", qty: " + Inventory.getInstance().getQuantity(b.getId()) + ")"));
                                ...
                                if (book.getStatus() == Book.Status.ABSENT) {
                                    System.out.println("Книга отсутствует на складе и не может быть заказана.");
                                    continue;
                                }
                                ...
                                // суммируем, если уже был выбран этот id
                            ...
                            System.out.println("Заказ создан. ID заказа: " + created.getId() +
                                    ", сумма: " + String.valueOf(created.getTotalPrice()) + " руб.");
                        }, null),

                        new MenuItem("Список заказов по дате", () ->
                                store.getOrdersSortedByDate().forEach(o -> System.out.println(o)), null),

                        new MenuItem("Список заказов по цене", () ->
                                store.getOrdersSortedByPrice().forEach(o -> System.out.println(o)), null),

                        new MenuItem("Список заказов по статусу", () ->
                                store.getOrdersSortedByStatus().forEach(o -> System.out.println(o)), null)
                ));
            ...
            case REPORTS:
                return new Menu("Отчёты", Arrays.asList(
                        ...
                        new MenuItem("Выручка за последний месяц", () -> {
                            ...
                            LocalDate to = now;
                            System.out.println("Выручка за последний месяц: " + String.valueOf(store.getRevenueInPeriod(from, to)) + " руб.");
                        }, null),
                        ...
                        new MenuItem("Количество выполненных заказов за последний месяц", () -> {
                            ...
                            LocalDate from = now.minusMonths(1);
                            LocalDate to = now;
                            System.out.println("Выполнено заказов за последний месяц: " + String.valueOf(store.getCompletedOrdersCountInPeriod(from, to)));
                        }, null)
                ));
